...
the kind of scoped-permission smart-wallet primitive [Swig](https://swig.so)
provides for Solana, or [Stellar's Smart Wallet SDK](https://developers.stellar.org/docs/build/apps/smart-wallets)
provides for Stellar. Casper has no native account-abstraction feature for
this, so it's implemented as a standalone contract: `PaymentGuard`.
...
signing each one** — the on-chain caps are the entire authorization model.
A payment above a cap, to an unregistered merchant, or to a paused/revoked
